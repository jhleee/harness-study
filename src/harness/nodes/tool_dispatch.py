...
1. AIMessage 의 모든 tool_calls 를 순회한다 (parallel tool calling 대응).
   각 호출마다 한 개의 ToolMessage 를 만들어 응답이 호출과 1:1 로 매칭되게
   한다 — 이를 빼먹으면 후속 LLM invoke 가 'tool call and result not match'
   로 거부된다.
2. 주입된 레지스트리에서 이름으로 도구를 찾고 실행한다 (try/except —
   도구가 터져도 턴 전체가 죽어서는 안 된다).
3. 결과를 문자열로 바꾼 길이가 OFFLOAD_THRESHOLD 자를 넘으면
   data/cache/out_<uuid>.txt 로 쓰고, 컨텍스트용 preview 500 자를 담은
   포인터 ToolMessage 를 돌려준다.
4. 4-튜플 트레이스 레코드(reasoning, tool, args, observation) 를 호출당
   하나씩 append 한다 — 4주차 self_improve 가 나중에 증류할 재료.
5. tool_call_count 를 호출 수만큼 증가.
...
def _all_tool_calls(state: HarnessState) -> list[dict]:
    ...
    *,
    ...
    def tool_dispatch(state: HarnessState) -> dict:
        ...
        for call in calls:
            ...
            tool_msgs.append(ToolMessage(content=final_content, tool_call_id=call_id))
